...
public class OldStyleModel {
	...
	static {
		...
		INTERMEDIARY_NOUN_PHRASE.setChunkFunction(ChunkFunction.INTERMEDIARY_NOUN_PHRASE);
		...
		INTERMEDIARY_VERB_PHRASE.setChunkFunction(ChunkFunction.INTERMEDIARY_VERB_PHRASE);
	...
	private static String phraseTagForCluster(List<Token> cluster) {
		for (Token token : cluster) {
			if(isBoundaryOfNounPhrase(token) || isIntermediaryNounPhrase(token)) {
				return "NP";
			} else if(isBoundaryOfVerbPhrase(token) || isIntermediaryVerbPhrase(token)) {
				return "VP";
			}
		}
		return null;
	...
	private static boolean isContinuation(List<List<Token>> tokenClusters, Token token) {
		...
		if( isBoundaryOfNounPhrase(lastToken) || isIntermediaryNounPhrase(lastToken) ) {
			if(isIntermediaryNounPhrase(token)) {
				return true;
			} else {
				return false;
			}
		} else if( isBoundaryOfVerbPhrase(lastToken) || isIntermediaryVerbPhrase(lastToken) ) {
