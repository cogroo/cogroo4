...
//		Span span = getSpan(sentence.getTokens());
	...
	private Chunk toNode(br.ccsl.cogroo.entities.Chunk chunk, SyntacticTag synt, Node parent)
	{
		Chunk c = new Chunk();
		c.setLevel(parent.getLevel() + 1);
//		Span span = getSpan(chunk.getTokens());
		c.setSyntacticTag(chunk.getSyntacticTag().toVerboseString());
//		c.details.add(SYNTACTIC_FUNCTION + chunk.getSyntacticTag().toVerboseString());
...
//		c.parent = parent;
	...
	private List<Token> toNode(List<br.ccsl.cogroo.entities.Token> tokenList, Node parent)
	{
...
//		t.details.add(MORPH_FUNCTION + token.getMorphologicalTag().toVerboseString());
