 ...
 */
public class ADChunkBasedHeadFinderSampleStream implements ObjectStream<ChunkSample> {
	 ...
	 */
	public ADChunkBasedHeadFinderSampleStream(ObjectStream<String> lineStream, boolean useCGTag, boolean expandME) {
	 ...
	 */
	public ADChunkBasedHeadFinderSampleStream(InputStream in, String charsetName, boolean useCGTag, boolean expandME) {
      this.useCGTags = useCGTag;
      this.expandME = expandME;
		try {
		...
		if (root != null) {
			...
			for (int i = 0; i < elements.length; i++) {
				if (elements[i].isLeaf()) {
					processLeaf((Leaf) elements[i], false, "O", sentence, tags, target, false);
				} else {
...
//	}
	...
	private void processLeaf(Leaf leaf, boolean isIntermediate, String phraseTag,
			List<String> sentence, List<String> tags, List<String> target, boolean isInherited) {
		String phrase;
		String pos;
		...
		if (leaf.getSyntacticTag() == null) {
		    ...
		    pos = ADChunkSampleStream.convertFuncTag(leaf.getFunctionalTag(), this.useCGTags);
		}
		tags.add(pos + "|" + phrase);
		if(!isInherited && "H".equals(leaf.getSyntacticTag()) && !"O".equals(phraseTag)) {
		  target.add("B-H");
		} else {
		  target.add("O");
		}
