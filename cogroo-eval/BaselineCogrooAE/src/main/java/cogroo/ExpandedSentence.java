    ...
    if(expandedSentence == null) {
      synchronized (this) {
        if(expandedSentence == null) {
          ...
          StringBuilder expSent = new StringBuilder(theSentence.getSentence().length());
          tokens = new ArrayList<Token>();
          for (int i = 0; i < theSentence.getTokens().size(); i++) {
            Token t = theSentence.getTokens().get(i);
            String lexeme = t.getLexeme();
            if(lexeme.length() < t.getSpan().length() && t.getSpan().getStart() != lastStart) {
              lexeme = t.getSpan().getCoveredText(theSentence.getSentence()).replace(" ", "_");
            }
            ...
            Token newToken = new TokenCogroo(t.getLexeme(), offset + t.getSpan().getStart());
            tokens.add(newToken);
          }
