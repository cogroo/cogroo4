...
public class PostPOSTagger implements ProcessingEngine {
  ...
  static {
    String[] pho = {"ex", "sota", "soto", "vice", "pré", "pós", "pró", "extra", "contra", "auto", "neo", "semi", "ultra", "supra", "intra"};
    PREFIXOS_HYPHENS = Collections.unmodifiableSet(new HashSet<String>(Arrays.asList(pho)));
  }
  ...
  private MorphologicalTag merge(MorphologicalTag a,
      MorphologicalTag b) {
    ...
    if(!isVariable(aClass)) {
      ...
      // prefer the noum
      if(aClass.equals(Class.NOUN) && bClass.equals(Class.NOUN) || aClass.equals(Class.ADJECTIVE) && bClass.equals(Class.ADJECTIVE)) {
        ret = b.clone();
