...
public class UimaMultiWordExp extends AnnotationService implements
  ...
  public void process(Sentence text) {
    ...
    FSIterator<Annotation> personIterator = cas.getAnnotationIndex(personType).iterator();
    FSIterator<Annotation> expIterator = cas.getAnnotationIndex(expType).iterator();
  ...
  private static void mergeTokens(List<Token> grouped, List<Integer> toMerge) {
    if(toMerge.size() > 0) {
      ...
      grouped.set(toMerge.get(0).intValue(), new TokenCogroo(lexeme, new Span(s,e)));
    }
  }
