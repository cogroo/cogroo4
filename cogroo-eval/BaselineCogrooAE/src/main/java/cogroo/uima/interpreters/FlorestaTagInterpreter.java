...
public class FlorestaTagInterpreter implements TagInterpreterI {
	...
	static {
		 ...
		 *  Chunk
		/* ********************************/
		Map<ChunkFunction, String> cenumElements = new HashMap<ChunkFunction, String>(7);
		...
		Map<String,List<ChunkFunction>> stringCElements = new HashMap<String,List<ChunkFunction>>(7);
		
		for (ChunkFunction tagE : k) {
			ArrayList<ChunkFunction> values = new ArrayList<ChunkFunction>();
			values.add(tagE);
			stringCElements.put(ENUM_CTAG_PARTS.get(tagE), Collections.unmodifiableList(values));
		}
		 ...
		 *  Syntactic
		/* ********************************/
		Map<SyntacticFunction, String> senumElements = new HashMap<SyntacticFunction, String>(3);
		...
		Map<String,List<SyntacticFunction>> stringSElements = new HashMap<String,List<SyntacticFunction>>(3);
		
		for (SyntacticFunction tagE : k1) {
			ArrayList<SyntacticFunction> values = new ArrayList<SyntacticFunction>();
			values.add(tagE);
			stringSElements.put(ENUM_STAG_PARTS.get(tagE), Collections.unmodifiableList(values));
		}
...
//		enumElements.put(Person.THIRD, "3S/P");
		...
		Map<String,List<Enum<?>>> stringMElements = new HashMap<String,List<Enum<?>>>(60);
...
// *	enumElements.put(Person.NONE_FIRST_THIRD, "0/1/3S");
		...
		ap.add(Class.PUNCTUATION_MARK);ap.add(Punctuation.BIN);
		stringMElements.put("$`", Collections.unmodifiableList(ap));
		stringMElements.put("$´", Collections.unmodifiableList(ap));
		
		
		ArrayList<Enum<?>> others = new ArrayList<Enum<?>>();
		others.add(Class.PUNCTUATION_MARK);others.add(Punctuation.NSEP);
	...
	//private final Map<String, MorphologicalTag> cache = new HashMap<String, MorphologicalTag>();
	...
	public MorphologicalTag parseMorphologicalTag(String tagString) {
		...
		for (String tag : tags) {
			if(MTAG_PARTS_ENUM.containsKey(tag)) {
			  ...
			  if("v-pcp".equals(tag)) {
			    m.setClazz(Class.VERB); m.setFiniteness(Finiteness.PARTICIPLE);
			  } else if("v-inf".equals(tag)) {
                m.setClazz(Class.VERB); m.setFiniteness(Finiteness.INFINITIVE);
              } else if("v-ger".equals(tag)) {
                m.setClazz(Class.VERB); m.setFiniteness(Finiteness.GERUND);
              } else if("v-fin".equals(tag)) {
                m.setClazz(Class.VERB); m.setFiniteness(Finiteness.FINITE);
...
//        CmdLineUtil.openInFile(new File("/Users/wcolen/Documents/wrks/corpus/FlorestaVirgem/FlorestaVirgem_CF_3.0_ad.txt")), "ISO-8859-1");
...
//        } else if(!con.equals(ori) && !con.contains("pnt") && !ori.contains("n-adj")) {
