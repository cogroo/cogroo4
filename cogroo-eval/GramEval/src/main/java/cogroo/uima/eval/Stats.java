...
public class Stats {
	...
	private Map<String, AtomicInteger> targetForOutcome = new HashMap<String, AtomicInteger>();
	  ...
	  public void addTP(String type) {
		  if(!tpForOutcome.containsKey(type)) tpForOutcome.put(type, new AtomicInteger());
		  
		  tpForOutcome.get(type).incrementAndGet();
	  }
	  
	  public void addFP(String type) {
		  if(!fpForOutcome.containsKey(type)) fpForOutcome.put(type, new AtomicInteger());
	...
	public void updateScores(List<Error> target, List<Error> predicted) {
		...
		for (Error t : target) {
			if(!m.containsKey(t.getType())) {
				m.put(t.getType(), new Pair<List<Error>, List<Error>>(new ArrayList<Error>(), new ArrayList<Error>()));
			}
		...
		for (Error p : predicted) {
			if(!m.containsKey(p.getType())) {
				m.put(p.getType(), new Pair<List<Error>, List<Error>>(new ArrayList<Error>(), new ArrayList<Error>()));
			}
		...
		for(String type : m.keySet()) {
			...
			mFMeasureForOutcome.get(type).updateScores(toArray(m.get(type).a), toArray(m.get(type).b));
		}
	}
...
//      set.addAll(targetForOutcome.keySet());
	...
	public String toString() {
		...
		int found = tp + fp;
		ret.append("processed " + senteces + " sentences with " + target + " grammar errors; found: " + found + "; correct: " + tp + ".\n");
	...
	public Map<String, Data> getData() {
		...
		set.addAll(fpForOutcome.keySet());
		for (String type : set) {
			res.put(type, new Data(type, mFMeasureForOutcome.get(type), 
					zeroOrValue(targetForOutcome.get(type)),
					zeroOrValue(tpForOutcome.get(type)),
					zeroOrValue(fpForOutcome.get(type))));
		}
	...
	static String format = "%8s: precision: " + n + ";  recall: " + n + "; F1: " + n + ".";
	...
	public static void main(String[] args) {
		...
		System.out.println(
				String.format(format, "ADVP", precision, recall, f, 10000, 10 ,0)
		);
		precision *= -1; recall = 0;
		System.out.println(
				String.format(format, "ADVP", precision, recall, f)
		);
		...
		System.out.println(
            String.format(tpfp, "ADVP", 1000, 2000)
    );
	}
	...
	private class Comp implements Comparator<String> {
		public int compare(String o1, String o2) {
			...
			if(mFMeasureForOutcome.containsKey(o1)) t1 += mFMeasureForOutcome.get(o1).getFMeasure();
			if(mFMeasureForOutcome.containsKey(o2)) t2 += mFMeasureForOutcome.get(o2).getFMeasure();
	   ...
	   private class Comp2 implements Comparator<String> {
	        public int compare(String o1, String o2) {
	            ...
	            if(fpForOutcome.get(o1) != null) fp1 = fpForOutcome.get(o1).get();
	            if(fpForOutcome.get(o2) != null) fp2 = fpForOutcome.get(o2).get();
	            
	            double tp1 = 0;
	            double tp2 = 0;
                
                if(tpForOutcome.get(o1) != null) tp1 = tpForOutcome.get(o1).get();
                if(tpForOutcome.get(o2) != null) tp2 = tpForOutcome.get(o2).get();
