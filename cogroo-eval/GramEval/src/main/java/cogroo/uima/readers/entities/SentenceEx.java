...
public class SentenceEx {
	...
	public String getId() {
		return sentence.getMetadata().substring(0, sentence.getMetadata().indexOf(" "));
	}
	private Pattern erPattern = Pattern.compile("c=(.*?)\\s+err=\"(.*?)\"\\s+rep=\"(.*?)\"");
	public List<GrEr> getGrammarErrors() {
		...
		while (m.find()) {
			...
			errors.add(new GrEr(m.group(1), m.group(2), m.group(3), this.start + b, this.start + b + err.length()));
		}
		return errors;
	}
