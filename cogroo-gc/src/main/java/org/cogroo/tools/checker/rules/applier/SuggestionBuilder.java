...
public class SuggestionBuilder {
	...
	public String[] getTokenSuggestions(Sentence sentence, List<Token> matched,
			Token next, Rule rule) {
		...
		for (Suggestion suggestion : rule.getSuggestion()) {
			String s = getTokenSuggestions(sentence, matched, next, suggestion,
					rule.getBoundaries());
			if (s != null && s.length() > 0)
				suggestionsAsString.add(s);
		}

		return suggestionsAsString.toArray(new String[suggestionsAsString
				.size()]);
	}

	public String[] getSyntacticSuggestions(Sentence sentence,
			List<SyntacticChunk> matched, Token next, Rule rule) {
		// Each suggestionsAsString position will contain a suggestion.
		Set<String> suggestionsAsString = new HashSet<String>();
		for (Suggestion suggestion : rule.getSuggestion()) {
			String s = getSyntacticSuggestions(sentence, matched, next,
					suggestion, rule.getBoundaries());
			if (s != null && s.length() > 0)
				suggestionsAsString.add(s);
		}
	...
	private String getTokenSuggestions(Sentence sentence, List<Token> matched,
			Token next, Suggestion suggestion, Boundaries boundaries) {

		// Gets only the tokens that are referred by the mistake. It considers
		// chunks and subjverb!
		Token[] underlinedTokens = SuggestionBuilder.tokensSubArray(matched,
				boundaries);

		String[] mistakenTokensAsString = SuggestionBuilder
				.tokensSubArrayAsString(underlinedTokens);

		// Tells if a token was replaced by an empty string.
		boolean replacedByEmptyString[] = new boolean[mistakenTokensAsString.length];

		// If can not determine an inflection for the replacement, reject the
		// suggestion.
		boolean reject = false;

		// Replaces.
		/*
		 * <Replace> work as follows. Lexeme TagReference Action 0 0 does
		 * nothing 0 1 gets primitive from the token in the sentence and queries
		 * for an inflection 1 0 replaces for the lexeme 1 1 gets primitive
		 * (<Lexeme>) from the <Replace> and queries for an inflection
		 */
		for (Replace replace : suggestion.getReplace()) {
			if (replace.getTagReference() == null
					&& replace.getLexeme() != null) { // L1, T0.
				...
				mistakenTokensAsString[replaceIndex] = RuleUtils
						.useCasedString(
								mistakenTokensAsString[(int) replace.getIndex()],
								replacement);
				replacedByEmptyString[replaceIndex] = replacement.equals("") ? true
						: false;
			} else { // T1.
				String[] primitive;
				int replaceIndex = (int) replace.getIndex();
				if (replace.getLexeme() != null) { // L1, T1.
					// Gets the primitive from Replace and queries the
					// dictionary for a replacement.
					String[] arr = { replace.getLexeme() };
					primitive = arr;
				} else { // L0, T1.
					// Gets the primitive from the sentence and queries the
					// dictionary for a replacement.
					primitive = matched.get((int) replace.getIndex())
							.getPrimitive();
				}

				// @ wildcard from CoGrOO 1.0 is tricky, very tricky.
				// The idea here is that if there was a @ in the suggestion, we
				// must get the gender or the
				// number of that particular token in order to correctly query
				// the dictionary.
				...
				if (replace.getReference() != null) {
					cloneTagMask = RuleUtils.createTagMaskFromReference(
							replace.getReference(), refMorphTag, null, null);
				} else {
		...
		if (!reject) { // If not reject, there's still hope... if reject, do not
						// swap nor concatenate.
			// Swaps.
			for (Swap swap : suggestion.getSwap()) {
				int a = (int) swap.getA();
				int b = (int) swap.getB();
				String temp = SuggestionBuilder
						.discardBeginningHyphen(mistakenTokensAsString[a]);
				mistakenTokensAsString[a] = SuggestionBuilder
						.discardBeginningHyphen(mistakenTokensAsString[b]);
			...
			for (ReplaceMapping replaceMapping : suggestion.getReplaceMapping()) {
				...
				if (replaceMapping.getKey().equals(
						mistakenTokensAsString[(int) index].toLowerCase())) {
					mistakenTokensAsString[(int) index] = RuleUtils
							.useCasedString(
									mistakenTokensAsString[(int) index],
									replaceMapping.getValue());
				}
			}

			// if the last token was a contraction, we should keep the other
			// part of it in the suggestion
			...
			for (int i = 0; i < mistakenTokensAsString.length; i++) {
				if (mistakenTokensAsString[i].startsWith("-")
						|| replacedByEmptyString[i]) {
	...
	private String getSyntacticSuggestions(Sentence sentence,
			List<SyntacticChunk> matched, Token next, Suggestion suggestion,
		...
		Span underlinedSpan = new Span(
				underlinedTokens[0].getSpan().getStart(),
				underlinedTokens[underlinedTokens.length - 1].getSpan()
						.getEnd());
		...
		for (Replace replace : suggestion.getReplace()) {
			if (replace.getTagReference() == null
				...
				mistakenTokensAsString[p.getStart()] = RuleUtils
						.useCasedString(
								mistakenTokensAsString[(int) replace.getIndex()],
								replacement);
				replacedByEmptyString[p.getStart()] = replacement.equals("") ? true
						: false;
			} else { // T1.
				...
				if (replace.getLexeme() != null) { // L1, T1.
					...
					// Gets the primitive from the sentence and queries the
					// dictionary for a replacement.
					for (Token toks : matched.get((int) replace.getIndex())
							.getTokens()) {
				...
				List<Token> originalTokens = matched.get(replaceIndex)
						.getTokens();
				...
				if (primitives != null) {
					for (int i = 0; i < primitives.size(); i++) {
						...
						RuleUtils.completeMissingParts(tm,
								ot.getMorphologicalTag());
						...
						String[] flexArr = flexList.toArray(new String[flexList
								.size()]); // Can be empty.
		...
		if (!reject) { // If not reject, there's still hope... if reject, do not
			...
			//
			// if(next.getSpan().equals(removed.getSpan())) {
			// mistakenTokensAsString[replacedByEmptyString.length - 1] =
			// next.getLexeme();
			// replacedByEmptyString[replacedByEmptyString.length - 1] = false;
			// }
			// }
			...
			StringBuilder suggestionAsStr = new StringBuilder(
					underlinedSpan.getCoveredText(sentence.getSentence()));
			...
			for (int i = mistakenTokensAsString.length - 1; i >= 0; i--) {
				if (mistakenTokensAsString[i] != null) {
					...
					if (previous.equals(s)) {
						// contraction?
						String contraction = ContractionUtility.toContraction(
								mistakenTokensAsString[i].toLowerCase(),
								mistakenTokensAsString[i + 1].toLowerCase());
						mistakenTokensAsString[i] = RuleUtils.useCasedString(
								underlinedTokens[i]
										.getSpan()
										.getCoveredText(
												sentence.getDocumentText())
										.toString(), contraction);
						s = new Span(previous.getStart(), previous.getStart()
								+ mistakenTokensAsString[i + 1].length());
					}
					previous = s;
					suggestionAsStr.replace(s.getStart() - offset, s.getEnd()
							- offset, mistakenTokensAsString[i]);
				}
				if (replacedByEmptyString[i] && i > 0
						&& i < mistakenTokensAsString.length - 1) {
	...
	// /**
	// * Gets the tokens from the <code>tokens</code> array which are between
	// <code>lower</code> and
	...
	// */
	// private static String[] tokensSubArrayAsString(List<Token> tokens, int
	// lower, int upper) {
	...
	/**
	 * Gets the tokens from the <code>tokens</code> array which are between
	 * <code>lower</code> and <code>upper</code> positions.
	 * 
	 * @param sentence
	 ...
	 * @return an array containing the desired tokens
	 */
	private static Token[] tokensSubArray(Sentence sentence, int lower,
			int upper, boolean considerChunk) {
	...
	private static Token[] tokensSubArray(List<Token> tokens,
			Boundaries boundaries) {
		int start = boundaries.getLower();
	...
	private static List<Token> getTokensRecursively(Token rootToken) {
		...
		if (rootToken.getChunk() != null
				&& rootToken.getChunk().getTokens().size() > 1) {
	...
	private static String getBestFlexedWord(String[] flex, Token original,
			TagMask tagMask) {
