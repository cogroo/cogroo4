 ...
 */
public class ADChunk2SampleStream implements ObjectStream<ChunkSample> {
     ...
     */
    public ADChunk2SampleStream(ObjectStream<String> lineStream) {
    ...
    String phraseTag = getChunkTag(node, inheritedTag, target.size());
    
    boolean inherited = false;
    if((phraseTag.equals(OTHER) /*|| phraseTag.equals(inheritedTag)*/) && inheritedTag != null) {
  ...
  protected void processLeaf(Leaf leaf, boolean isIntermediate, String phraseTag,
        ...
        if (leaf.getSyntacticTag() == null) {
            ...
            tags.add(ADChunk2SampleStream.convertFuncTag(leaf.getFunctionalTag(), false));
        }
        target.add(chunkTag);
    }
  ...
  protected String getChunkTag(Node node, String parent, int index) {
    ...
    if("adjp".equals(phraseTag) && !"NP".equals(parent)) {
      phraseTag = "np";
    }
