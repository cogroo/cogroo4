 ...
 * </table>
 */
public final class POSTaggerWithAdditionalContext extends CasAnnotator_ImplBase {
   ...
   * constructor.
   */
  public POSTaggerWithAdditionalContext() {
  ...
  public void typeSystemInit(TypeSystem typeSystem) throws AnalysisEngineProcessException {
    ...
    this.additionalContextFeature = AnnotatorUtil.getRequiredFeatureParameter(this.context, this.tokenType,
        "opennlp.uima.AdditionalContextFeature", CAS.TYPE_NAME_STRING);

    this.lexemeFeature = AnnotatorUtil.getRequiredFeatureParameter(this.context, this.tokenType,
  ...
  public void process(CAS tcas) {
    ...
    for (AnnotationIteratorPair annotationIteratorPair : comboIterator) {
      ...
      final List<String> additionalContextList = new LinkedList<String>();

      for (AnnotationFS tokenAnnotation : annotationIteratorPair.getSubIterator()) {

        sentenceTokenAnnotationList.add(tokenAnnotation);

        sentenceTokenList.add(tokenAnnotation.getFeatureValueAsString(lexemeFeature));
        
        additionalContextList.add(tokenAnnotation.getFeatureValueAsString(additionalContextFeature));
      }
      
      String[] toks = sentenceTokenList.toArray(new String[sentenceTokenList.size()]);
      
      String[][] additionalContext = new String[toks.length][2];
      
      int i = 0;
      for (String ac : additionalContextList) {
        if(ac != null) {
          if(ac.equals("P")) {
            additionalContext[i][1] = ac;
          } else {
            additionalContext[i][0] = ac;
          }
        }
        i++;
      }
      
      final List<String> posTags = Arrays.asList(this.posTagger.tag(toks, additionalContext));
